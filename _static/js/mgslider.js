...
mgslider.prototype.change_from_slider = function (slider) {
    // Initialize the value history if not already initialized
    this.valueHistory = this.valueHistory || ""; 

    // Append the current value to the history
    var currentValue = this.from_internal(slider.value);
    
    // Check if valueHistory is empty or if the current value is not already in the history
    if (this.valueHistory === "" || !this.valueHistory.endsWith(currentValue + ",")) {
        this.valueHistory += currentValue + ",";
    }

    // Update the slider's value in the hidden input
    document.getElementById(this.id("input")).value = currentValue;

    // Update the element whose name is slider + "_list" with the valueHistory
    var listElement = document.getElementsByName(this.field + "_list")[0];
    if (listElement) {
        listElement.value = this.valueHistory;
    }
